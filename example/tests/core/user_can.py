...
class UserCanTest(M2CoreAsyncHTTPTestCase):
    ...
    def test_authenticated(self):
        response = self.fetch_json('/authenticated', user_permissions={PlatformPermissions.AUTHORIZED, })
        self.assertEqual('ok', response['data'])

        response = self.fetch_json(
            '/authenticated-but-not-enough',
            user_permissions={PlatformPermissions.AUTHORIZED, },
    ...
    def test_authenticated_but_not_enough(self):
        response = self.fetch_json(
            '/authenticated-but-not-enough',
            user_permissions={PlatformPermissions.AUTHORIZED, PlatformPermissions.VIEW_SOME_INFO, },
            expected_codes={http_statuses['WRONG_CREDENTIALS']['code'], }
        )
        self.assertEqual(None, response['data'])
